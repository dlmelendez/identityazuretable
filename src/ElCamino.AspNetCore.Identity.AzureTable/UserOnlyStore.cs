...
namespace ElCamino.AspNetCore.Identity.AzureTable
{
        ...
        where TContext : IdentityCloudContext, new()
    {
        ...
        public UserOnlyStore(TContext context, IdentityConfiguration config) : base(new IdentityErrorDescriber())
        {
        ...
        protected async Task<TUserLogin> FindUserLoginAsync(string userId, string loginProvider, string providerKey)
        {
            ...
            var result = _userTable.ExecuteQueryAsync(tq);
            var log = await result.FirstOrDefaultAsync().ConfigureAwait(false);
            if (log != null)
            {
        ...
        public async Task<IEnumerable<TUser>> FindAllByEmailAsync(string plainEmail)
        {
            ...
            return users.Where(user => KeyHelper.GenerateRowKeyUserEmail(plainEmail) == KeyHelper.GenerateRowKeyUserEmail(user.Email));
        }
        ...
        {
            var user = await _indexTable.ExecuteQueryAsync(queryUser).FirstOrDefaultAsync();
            if (user != null)
            {
        ...
        public virtual async Task RemoveClaimAsync(TUser user, Claim claim)
        {
            ...
            if (local != null)
            {
                ...
                var tasks = new Task[]
                {
                    _indexTable.ExecuteAsync(TableOperation.Delete(CreateClaimIndex(KeyHelper.GenerateRowKeyUserId(ConvertIdToString(user.Id)), claim.Type, claim.Value))),
        ...
        public override async Task SetEmailAsync(TUser user, string email, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                await DeleteEmailIndexAsync(KeyHelper.GenerateRowKeyUserId(ConvertIdToString(user.Id)), user.Email).ConfigureAwait(false);
            }
            user.Email = email;
        }
        ...
        protected async Task DeleteEmailIndexAsync(string userId, string plainEmail)
        {
            ...
            var indexes = _indexTable.ExecuteQueryAsync(tq);

            await foreach (DynamicTableEntity de in indexes)
            {
